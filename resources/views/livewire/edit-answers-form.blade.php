        ...
        @foreach ( $answers  as $key => $answer)
            <tr>
                ...
                <td class="pl-2 w-[80%] border-2">
                    <textarea row="2" class="textarea text-wrap w-full bg-zinc-50" wire:key.live="$key"
                ...
                <td class="text-right align-middle border-2">
                    <x-form action="/doAnEdit">
                        <button
                            class="text-white bg-slate-700 hover:bg-teal-500 p-2 md:px-4 rounded-md inline-block mx-auto text-xs md:text-sm w-auto">
                            Update
                        </button>
                    </x-form>
                </td>
