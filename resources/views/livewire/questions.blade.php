<div>
        ...
        <div class="col-span-7">
          <blockquote class="speech bubble">
            ...
            <h3 class="text-lg font-semibold self-start" :class="{{$question->id}} == 1 ? '' : 'hidden'">
                {{session('cust.first_name')}}, you gave {{ session('rating')[0] }} {{ Str::plural('star',session('rating')[0]) }}&hellip; 
            </h3>
            <p>
                {{ $question->question }}
            </p>
    ...
    <div>
        <h2 class="self-start">
            <img src="{{ asset('https://cdn.mojoimpact.com/twoshakes/new-guru-256.webp')}}" class="h-8 w-8 inline-block" alt="guru icon">
        ...
        <form wire:submit="handleHelp" class="bg-slate-100 w-full p-3 border-t border-slate-200 flex space-x-2">
            <div class="bg-white rounded-full grow relative flex items-center h-10 focus-within:ring-2 ring-inset ring-blue-500">
                <input wire:model="ask" class="bg-transparent rounded-full grow px-4 py-2 text-sm h-10 focus:outline-0" placeholder="I'm not sure what to say..." />
            ...
            <div class="h-[15vh] w-full mb-5 border rounded-lg bg-gradient-to-t from-slate-100 p-6 flex space-y-1.5 overflow-scroll flex-col-reverse">
            <div class="flex flex-col">
                @foreach($aiMsg as $key => $msg)                
                    ...
                    @if ($msg['role'] === 'assistant')
                        <livewire:chat-response :key="$key" :questionNo="$question->id" :messages="$aiMsg" :helpText="$aiMsg[$key - 1]" />                        
                    @endif
                @endforeach
            </div>
        </div>
