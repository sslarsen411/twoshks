        ...
        <div class="p-4 flex-col border-2">
            ...
            <div id="ph_area" class="w-full self-start {{$ckCallMe?'':'hidden'}}">
                <div class="relative z-0 w-3/4 mb-5 group">
                    ...
                    <div class="{{ $ckCallMe && $phone == null? '': 'hidden' }}">
                        <span class="error text-xs">Enter a valid US phone number</span>
                    </div>
                    @error('phone') <span class="error text-xs">{{ $message }}</span> @enderror
                    <label for="phone"
                           class="ml-2 my-2 peer-focus:font-medium absolute text-sm text-gray-500 duration-300
